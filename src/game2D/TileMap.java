	...
	public void draw(Graphics2D g, int xoff, int yoff)
	{
		...
		for (int r=0; r<mapHeight; r++)
		{
			for (int c=0; c<mapWidth; c++)
			{
				...
				//Use 0 to draw only what's on screen. -64 allows me to draw 2 tiles to the left and 2 tiles to the top outside of the screen!
                //Remove these lines completetly to draw the whole map! (for collision map!)
				if (xc+tileWidth < -tileWidth * 2 || xc >= rect.x + rect.width) continue;
                if (yc+tileHeight < -tileHeight * 2 || yc >= rect.y + rect.height) continue;
